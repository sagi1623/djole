...
export class CommentListService 
{
    ...
    getById(accId: number, appId: number): Observable<any>
    {
        ...
        return this.http.get(this.urlProviderService.getURL() + `api/Comments?$filter=AccommodationId eq ${accId} and CustomerId eq ${appId} &$expand=Customer, Accommodation`).map(res => res.json());
    }
